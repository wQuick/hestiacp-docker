...
if [ "$HESTIA_B" = true ]; then
	...
	for BUILD_ARCH in $arch; do
		...
		if [ "$BUILD_DEB" = true ]; then
			...
			#	/tmp/hestiacp-src/hestiacp-localsrc

			cd $BUILD_DIR/hestiacp-$branch_dash
			#echo `pwd`
			#echo `ls`
			#wget https://raw.githubusercontent.com/hestiacp/hestiacp/main/package.json -O package.json
			#wget https://raw.githubusercontent.com/hestiacp/hestiacp/main/package-lock.json -O package-lock.json
